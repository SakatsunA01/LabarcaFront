  ...
  <div class="flex items-center justify-center min-h-screen bg-gray-100">
    <div class="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
      ...
      <p class="text-sm text-center text-gray-600">
        ...
        <router-link to="/register" class="font-medium text-indigo-600 hover:text-indigo-500">Regístrate</router-link>
      </p>
    </div>
  </div>
</template>
...
const handleSubmit = async () => {
  try {
    ...
    // Después de un login exitoso, decide a dónde redirigir
    if (authStore.isAdmin) {
      router.push('/admin');
    } else {
      router.push('/'); // Redirige al home para usuarios no-admin
    }
  } catch (error) {
    // El error ya se maneja y se muestra desde el store
    console.error('Fallo en el handleSubmit de login:', error);
  }
};
</script>
